...
import {
  ...
  ChevronDown, ChevronUp, Check, Loader2, X, RefreshCw
} from "lucide-react";
...
function buildSystemPrompt(theme, trigger, duration) {
...
Target length: ${duration.label} spoken video (~${duration.words} words total across hook + body + CTA, at natural spoken pace). Stay close to this word count — it determines whether the script fits the runtime.
...
export default function App() {
  ...
  const selectedTrigger = TRIGGERS.find((t) => t.id === triggerId) || null;
  const selectedDuration = DURATIONS.find((d) => d.id === durationId) || null;
  const selectedCount = [themeId, triggerId, durationId].filter(Boolean).length;
  ...
  async function generateScript() {
    ...
    try {
      const sys = buildSystemPrompt(selectedTheme, selectedTrigger, selectedDuration);
            ...
            <div className="mb-7">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                {THEMES.map((t) => {
                  ...
                  return (
                    <button key={t.id} onClick={() => setThemeId(t.id)} className={`${cls} rounded-xl p-3`}>
